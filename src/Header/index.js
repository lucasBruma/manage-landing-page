...
function Header({ showMenu, openModal, sections, toggleModal }) {
    ...
    return ( 
        ...
        <Flex 
            ...
            px='6rem' 
            py='3rem'
            _after={{content:"''", background:`no-repeat url(${imgBg})` , position: 'absolute',left: '46%',top: '-110px',zIndex: '-2', width: '100%', height: '100%' }}
        >
            <Image src={logo} pt='5px'/>
            <HStack>
                {sections.map((section, index)=>{
                    return <Link to={`${section.href}`} spy={true} smooth={true} offset={50} duration={500}>
                        <Button variant='outline' key={index}>{section.name}</Button>
                    </Link>
                })}
            </HStack>
        ...
        <Flex 
            ...
            px='1rem' 
            py='3rem'
            _after={{content:"''", background:`center/104% no-repeat url(${imgBg})` , position: 'absolute',left: '12%',top: '-5%',zIndex: '-2', width: '134%', height: '18%' }}
        >
